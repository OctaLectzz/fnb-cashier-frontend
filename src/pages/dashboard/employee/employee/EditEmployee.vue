    ...
    <q-form @submit="editData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">{{ $t('dashboard.employee.employee.editText') }}</div>
          <q-space />
        ...
        <q-card-section class="scroll">
          <q-stepper v-model="step" ref="stepper" color="primary" animated>
            <q-step :name="1" :title="$t('dashboard.employee.employee.step1') + $t('public.requiredText')" icon="settings" :done="step > 1" class="scroll" style="height: 66vh">
            ...
            <q-step :name="2" :title="$t('dashboard.employee.employee.step2')" :caption="$t('public.optionalText')" icon="create_new_folder" :done="step > 2" class="scroll" style="height: 66vh">
              <!-- KTP -->
              <div class="col-md-10 col-xs-12 q-pa-sm">
                <div class="row">
                  <div class="col-md-2 col-xs-12">
            ...
            <q-step :name="3" :title="$t('dashboard.employee.employee.step3')" :caption="$t('public.optionalText')" icon="add_comment" :done="step > 3" class="scroll" style="height: 66vh">
              <!-- Employment Status -->
              <div class="col-md-10 col-xs-12 q-pa-sm">
                <div class="row">
                  <div class="col-md-2 col-xs-12">
              ...
              <div class="col-md-10 col-xs-12 q-pa-sm">
                <div class="row">
                  ...
                  <div class="col-md-10 col-xs-12 q-px-sm">
                    <q-input v-model="data.date_joined" mask="date" :rules="rules.dateJoined" outlined dense required>
                      <template v-slot:prepend>
                        <q-icon name="event" class="cursor-pointer">
                          <q-popup-proxy transition-show="scale" transition-hide="scale" cover>
              ...
              <div v-if="data.employment_status !== 'permanent'" class="col-md-10 col-xs-12 q-pa-sm">
                <div class="row">
                  ...
                  <div class="col-md-10 col-xs-12 q-px-sm">
                    <q-input v-model="data.end_date" mask="date" outlined dense required>
                      <template v-slot:prepend>
                        <q-icon name="event" class="cursor-pointer">
                          <q-popup-proxy transition-show="scale" transition-hide="scale" cover>
            ...
            <template v-slot:navigation>
              <q-stepper-navigation>
                <q-btn color="primary" :label="$t('dashboard.employee.employee.backBtn')" class="q-mt-md" :disable="step == 1" @click="$refs.stepper.previous()" />
                <q-btn color="primary" :label="$t('dashboard.employee.employee.continueBtn')" class="float-right q-mt-md" :disable="step == 3" @click="$refs.stepper.next()" />
              </q-stepper-navigation>
            </template>
          </q-stepper>
        </q-card-section>
...
const data = ref({
  ...
  nip: item.nip,
  avatar: item.avatar,
  avatarPreview: url + '/employees/avatars/' + item.avatar,
  name: item.name,
  email: item.email,
  ...
  schedule: item.schedule.name + ' (' + item.schedule.start_time + ' - ' + item.schedule.end_time + ')',
  ktp: item.ktp,
  ktp_image: item.ktp_image,
  ktpPreview: url + '/employees/ktps/' + item.ktp_image,
  dob: item.dob,
  gender: item.gender,
  domicile: item.domicile,
  address: item.address,
  employment_status: item.employment_status,
  date_joined: item.date_joined_edit,
  end_date: item.end_date,
  bpjs_tk_number: item.bpjs_tk_number,
  bpjs_tk_card: item.bpjs_tk_card,
  bpjsTkPreview: url + '/employees/bpjstks/' + item.bpjs_tk_card,
  bpjs_health_number: item.bpjs_health_number,
  bpjs_health_card: item.bpjs_health_card,
  bpjsHealthPreview: url + '/employees/bpjshealthes/' + item.bpjs_health_card,
...
const editData = async () => {
  ...
  try {
    ...
    toast.success(t('dashboard.employee.employee.successEditMsg'))
    emits('edited')
  } catch (error) {
    console.error('Error submitting form:', error)

    toast.error(error.response.data.message || t('dashboard.employee.employee.failedEditMsg'))
  }
  loading.value = false
}
</script>
