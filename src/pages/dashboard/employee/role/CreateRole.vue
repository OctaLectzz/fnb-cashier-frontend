    ...
    <q-form @submit="createData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">{{ $t('dashboard.employee.role.createText') }}</div>
          <q-space />
...
const createData = async () => {
  loading.value = true
  try {
    await useRoleStore().create(data.value)

    toast.success(t('dashboard.employee.role.successCreateMsg'))
    emits('created')
  } catch (error) {
    console.error('Error submitting form:', error)

    toast.error(error.response.data.message || t('dashboard.employee.role.failedCreateMsg'))
  }
  loading.value = false
}
</script>
