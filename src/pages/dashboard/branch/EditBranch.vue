    ...
    <q-form @submit="editData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">{{ $t('dashboard.branch.editText') }}</div>
          <q-space />
...
const editData = async () => {
  ...
  try {
    ...
    toast.success(t('dashboard.branch.successEditMsg'))
    emits('edited')
  } catch (error) {
    console.error('Error submitting form:', error)

    toast.error(error.response.data.message || t('dashboard.branch.failedEditMsg'))
  }
  loading.value = false
}
</script>
