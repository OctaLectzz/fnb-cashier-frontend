    ...
    <q-form @submit="createData">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">{{ $t('dashboard.main.product.createText') }}</div>
          <q-space />
...
const rules = ref({
  ...
  sku: [
    ...
    (v) => {
      if (typeof v === 'string') {
        if (products.value) {
          return !products.value.some((product) => product.sku.toLowerCase() === v.toLowerCase()) || t('dashboard.main.product.validate.skuAlready')
        }
      }
      return true
    }
...
const createData = async () => {
  ...
  try {
    ...
    toast.success(t('dashboard.main.product.successCreateMsg'))
    emits('created')
  } catch (error) {
    console.error('Error submitting form:', error)

    toast.error(error.response.data.message || t('dashboard.main.product.failedCreateMsg'))
  }
  loading.value = false
}
